...
export default function MetricCard({ metric, sub, prior, higherIsBetter = true, footerPinned = true }: {
  metric: Metric; sub?: string; prior?: Metric | null; higherIsBetter?: boolean;
  /** false = sub/caveat flow directly under the value instead of pinning to the card bottom */
  footerPinned?: boolean;
}) {
  const delta = computeDelta(metric, prior);
  const good = delta ? delta.up === higherIsBetter : false;
  return (
    <div className="panel panel-hover flex h-full flex-col p-4">
      ...
      {(sub || metric.caveat) && (
        <div className={footerPinned ? "mt-auto" : ""}>
