...
| Facts to metrics (EBITDA, DSCR, ROCE, runway, FCF bridge) | Pure-function metrics engine, 34 unit tests | Financial calculations must be exact and auditable |
...
3. **Honest finance on a pre-profit micro-cap.** DSCR is -50.3x at FY25 and is shown
   ...
   guidance) rather than suppressed. Same treatment for ROCE and gearing on negative
...
chat and re-extraction activate. Set `AI_COMMENTARY=0` to stop commentary generation
(cached commentary is still served).
...
2. **Deterministic identity checks** (21 accounting identities): balance sheet balances;
   ...
   channel counts sum to the stated total. Run them yourself:
   `python -m pipeline.run --validate`. A 22nd check records a disclosure-coverage
...
backend/
  ...
  tests/          34 tests: metrics vs hand-computed values, validation, API e2e
frontend/
  src/app/        login + dashboard routes (overview, growth, profitability, cash,
                  solvency, returns, scenario, documents, chat)
